...
extern float limit_top;
extern float limit_bottom;
extern float limit_right;
extern float limit_left;
...
void LoadConfig() {
  ...
  if(version_number==3) {
    ...
    robot_uid=EEPROM_readLong(ADDR_UUID);
    adjustSpoolDiameter((float)EEPROM_readLong(ADDR_SPOOL_DIA1)/10000.0f);   //3 decimal places of percision is enough
    limit_top = (float)EEPROM_readLong(ADDR_LIMIT_TOP)/10000.0f;
    limit_bottom = (float)EEPROM_readLong(ADDR_LIMIT_BOT)/10000.0f;
    limit_right = (float)EEPROM_readLong(ADDR_LIMIT_R)/10000.0f;
    limit_left = (float)EEPROM_readLong(ADDR_LIMIT_L)/10000.0f;
  } else {
...
//------------------------------------------------------------------------------
void SaveMachineLimits() {
  EEPROM_writeLong(ADDR_LIMIT_TOP,limit_top*10000);
  EEPROM_writeLong(ADDR_LIMIT_BOT,limit_bottom*10000);
  EEPROM_writeLong(ADDR_LIMIT_R,limit_right*10000);
  EEPROM_writeLong(ADDR_LIMIT_L,limit_left*10000);
}
